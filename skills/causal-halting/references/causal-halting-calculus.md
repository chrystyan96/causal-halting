...
CHC-1 should add recursive CHC definitions with causal effect summaries as fixed points:

```text
G_f = body_graph(f, G_f)
```

The technical question becomes whether the fixed-point causal graph has an acyclic solution.

CHC-2 should add controlled higher-order code with effect-polymorphic causal types.
